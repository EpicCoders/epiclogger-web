@utils = angular.module("UtilsModule", [])

@utils.service "Utils", ["$rootScope", ($rootScope) ->

  
  # -------------------------------------
  # making clearance of unwanted attributes of objects
  # -------------------------------------
  clearResource = (properties,resource)=>
  ...
  resourceAttributes = (resource)=>
    attrs = {}
    (attrs[name]=value) for own name, value of resource
    attrs

  # -------------------------------------
  # we update an object elements with the new given elements
  # -------------------------------------
  updateObject = (elements,object)=>
    for index,value of elements
      object[index] = value
  ...
  # -------------------------------------
  # we use this function to camelize a string
  # -------------------------------------
  camelize = (key) ->
  ...
  # -------------------------------------
  # we use this method to camelize object attributes
  # -------------------------------------
  camelizeObject = (data) ->
    newKey = undefined
    if data and angular.isObject(data)
  ...
  # -------------------------------------
  # some simple method that adds a noty message with the posibility to parse 
  # rails native error messages
  # -------------------------------------
  addAlert = (msg,type='error')->
    ...
    noty({text:msg,type:type,timeout:10000})
  ...
  clickEventType = ()->
    return (if mobileCheck() then "touchstart" else "click")
...
# -------------------------------------
# directive to prevent default behaviour of anchors
# -------------------------------------

@utils.directive "noHref", ->
  (scope, element, attrs) ->
    $(element).click (event) ->
